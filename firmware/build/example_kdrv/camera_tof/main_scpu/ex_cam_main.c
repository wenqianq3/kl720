...
static uint32_t myCamera_example_open(uint8_t cam_idx, uint32_t width, uint32_t height, uint32_t pixelformat)
{
    ...
    buffer_addr[0] = kmdw_ddr_reserve(1542400);
    buffer_addr[1] = kmdw_ddr_reserve(1542400);
...
******************************************************************************/
extern volatile uint32_t imgcnt;
int main(void)
{
    static uint32_t imgcnt_tmp;
    /* SDK main init */
    sdk_main();

    masked_outw(H2X0_REG_BASE, 4, 0xC);
    masked_outw(H2X1_REG_BASE, 4, 0xC);
    kmdw_camera_init();
    myCameratest();
    kmdw_camera_set_flip(0, 1);
    kmdw_camera_set_mirror(0, 1);
    //kmdw_camera_set_inc(1, 1);
    while (1)
    {
        kdrv_timer_delay_ms(1000);
        kmdw_printf(">> Working ... diff imgcnt =  = %d, imgcnt = %d\n", imgcnt - imgcnt_tmp, imgcnt);
        imgcnt_tmp=imgcnt;
